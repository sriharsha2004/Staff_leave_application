...
<body>
    ...
    <a href="Coordinator_login.jsp" id="logout">Logout</a>
    <ul>
        <%
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/Project", "root", "nms@2000");
            PreparedStatement ps = con.prepareStatement("SELECT * FROM Staff WHERE appliedstatus=1");
            ...
            while (rs.next()) {
                ...
                <li><%= rs.getString("sname") %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    ...
                    <button class="btn staff" type="button" onclick="updateCoordinator(<%= rs.getInt("sno") %>);">Forward</button>
                    <button class="btn staffreject" type="button" onclick="updaterejectcoordinator(<%= rs.getInt("sno") %>);">Reject Leave</button>
                </li>
                <%
            }
            con.close();
    ...
    <script>
        function updateCoordinator(snoValue) {
            // Create an XMLHttpRequest object
            var xhttp = new XMLHttpRequest();

            // Configure the request
            xhttp.open("POST", "coordinatorchangestatus.jsp", true);
            ...
            xhttp.onreadystatechange = function() {
                if (xhttp.readyState === 4 && xhttp.status === 200) {
                    ...
                    alert("Leave forwarded to director successfully!");
        ...
        function updaterejectCoordinator(snoValue) {
            // Create an XMLHttpRequest object
            var xhttp = new XMLHttpRequest();

            // Configure the request
            xhttp.open("POST", "coordinatorrejectchangestatus.jsp", true);
            xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
